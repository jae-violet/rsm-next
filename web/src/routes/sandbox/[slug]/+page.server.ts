...
export const _query = gql`
	query Sandbox($slug: String!) {
		sandbox(filter: { slug: { _eq: $slug }}) {
			slug
			page_content {
				item {
					__typename
...
export const load: PageServerLoad = async ({ params }) => {
	const res = await cmsClient.Sandbox({ slug: params.slug });

	if (res.sandbox.length === 0) {
		throw error(404);
	}

	return { sandbox: res.sandbox[0] };
};
