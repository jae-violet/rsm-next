...
export const _query = gql`
	query BookSlug($slug: String!) {
		books(filter: { slug: { _eq: $slug }, visibility: { _neq: "archived" } }) {
			slug
			book_description
			hero_image {
				title
...
export const load: PageServerLoad = async ({ params }) => {
	const res = await cmsClient.BookSlug({ slug: params.slug });
